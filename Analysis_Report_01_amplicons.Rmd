...
```{r, Actinobacteria_key_identification}
# Filter the most commonly found species, Actinobacteria
# and examine the sample source of where Actinobacteria was found
melted_obj %>%
...
```{r, male-sample_type_Phylum}
# To examine for male indivudals how phylums
# Matches up in between skin ans surface samples
melted_obj %>%
  filter(Sex == "male") %>%
  ggplot(aes(x = Phylum,
             y = Abundance,
...
```{r, space_bar-phylum-sex}
# use space_bar which is the most common sampe sourece
# to identify the association between phylum and sex
melted_obj %>%
  filter(sample_source == "Space_bar") %>%
  ggplot(aes(x = Phylum,
             y =  Abundance, 
             fill = host_subject_id,
             color = host_subject_id,
             shape = Sex)) +
