...
```
**Figure1**: What are the Phylum identified from each individuals samples. Including bother skin as the fingertip and surface as the keyboard. As the Figure presented, for the keyboard samples for all the individuals, the most commonly found Phylum across all the indivudals is Actinobacteria, present in organge color. The second commnoly found bacteria Phylum is Firmicutes. This finding is consistant with fingertip samples, however, there are individuals does not has fingertip sample. Including F1 who is a female individual; for L1, L3, R1, U1, U2, U3 the sex of the individual is not applictable.


```{r, Actinobacteria_key_identification-barplot}
...
melted_obj %>%
  ...
  facet_grid(Phylum ~.)
```
**Figure2**: For the most two commly found bacteria phylum across all the individual samples: Actinobacteria, Firmicutes. What how frequently these two bacteria are found on each keys and fingertip is illustrated. As an result, only male individuals M2, M3, M9 has all keys sampled. For other individuals, only space bar is sampled. 
...
melted_obj %>%
  ...
  theme(axis.text.x = element_text(angle = 90,
                                    hjust = 1)) +
  ggtitle("Genus abundance in Propionibacteriaceae family on M2, M3, M9")
```
...
subset1_Genus <- subset_taxa(phyloseq_obj, Genus == "Propionibacterium")
...
subset2_male <- subset_samples(subset1_Genus, host_subject_id %in% c("M2","M3","M9"))
```
